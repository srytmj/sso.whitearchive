...
@section('content')
    ...
    @elseif(isset($invitation))
        <div class="alert alert-info alert-soft mb-6 text-sm animate-slide-up" role="alert">
        ...
        <form method="POST" action="{{ route('invite.store') }}" novalidate>
            ...
            <div class="space-y-4">
                <div>
                    <label class="block text-sm font-medium text-base-content/70 mb-1">{{ __('auth.full_name') }}</label>
                ...
                <button type="submit" class="btn btn-neutral w-full active:scale-95">{{ __('auth.activate_account_button') }}</button>
            </div>
        </form>
    @endif
@endsection
