...
@section('subtitle', __('auth.reset_password_subtitle'))
...
@section('content')
    ...
    <form method="POST" action="{{ route('password.update') }}" novalidate>
        ...
        <div class="space-y-5">
            ...
            <div>
                <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1">{{ __('auth.new_password') }}</label>
                <div x-data="{ show: false }" class="relative">
                    <input :type="show ? 'text' : 'password'" name="password" autocomplete="new-password" autofocus
            ...
            <button type="submit" class="w-full inline-flex items-center justify-center px-4 py-2 bg-zinc-900 dark:bg-zinc-100 text-white dark:text-zinc-900 text-sm font-medium rounded-lg hover:bg-zinc-800 dark:hover:bg-white transition-colors">{{ __('auth.reset_password_button') }}</button>
        </div>
    </form>
@endsection
